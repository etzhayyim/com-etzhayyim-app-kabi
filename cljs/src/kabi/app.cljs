(ns kabi.app
  "com-etzhayyim-app-kabi appview — reagent + re-frame, view built from
  ...
  - `:app/route-count` / `:app/routes` / `:app/vars` — the Svelte constant
    carried stale values (`routeCount: 0`, `routes: []`, `vars: []`), out
    of sync with the two routes and eight var keys `wrangler.jsonc`
    actually declares. This port reports what wrangler.jsonc actually
    declares, not the stale constant.
  ...
  components or ext-rules change) with:
    ...
    (spit \"public/index.html\"
          (page/->page {:title \"etzhayyim-project-kabi\"
                         :lang \"ja\"
                         :description \"com-etzhayyim-app-kabi — kabi (カビ) mycelium network layer appview (reagent + re-frame + jp-go-dds).\"
                         :css (slurp (io/resource \"jp_go_dds/dds.css\"))}
                        [:div {:id \"app\"} \"etzhayyim-project-kabi loading…\"]
...
(def default-db
  {:app/title "Ai etzhayyim Project Kabi"
   :app/project "etzhayyim-project-kabi"
   :app/name "etzhayyim-project-kabi"
   :app/kind "cloudflare surface"
   :app/route-count 2
   :app/routes ["k4b100m1.etzhayyim.com/*" "kabi.etzhayyim.com/*"]
   :app/vars ["AGENTGATEWAY_MCP_ROUTER_URL" "APP_CAPABILITIES" "APP_DESCRIPTION"
              ...
              "APP_UI_TYPE"]
   :app/xrpc? false
   :app/relative-path "cljs/src/kabi/app.cljs"})
